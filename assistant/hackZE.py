...
from SourceVenom import bot, Tayson
#By Source venom @Ve_No_m_S
...
'''
mm = '''
قم بلأنضمام الى قناة  فينوم @Ve_No_m_S
'''
...
keyboard = [
  ...
  [
    Button.url("سورس  فينوم 🔱", "https://t.me/Ve_No_m_S")
    ]
]
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        result = None
        ze = Bot_Username.replace("@", "")
        query = event.text
        await bot.get_me()
        if query.startswith("هاك") and event.query.user_id == bot.uid:
            buttons = Button.url(" اضغط هنا عزيزي ", f"https://t.me/{ze}?start=hack")
  ...
  if event.sender_id == bot.uid:
      async with bot.conversation(event.chat_id) as x:
        keyboard = [
          ...
          [
            Button.url("المطور", "https://t.me/S_Ml10")
            ]
        ]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("لقد تم حذف القناة/الكروب شكرا لأستخدامك  فينوم.", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if i == True:
          ...
          await event.reply(f"حدث خطأ قم بتوجيه الرسالة للمطور @S_Ml10\n{i}")
...
async def start(event):
    keyboard = [
      ...
      [
        Button.url("القناة", "https://t.me/Ve_No_m_S")
        ]
    ]
